...
function create_div(dleft, dtop, dwidth, dheight)
{
    ...
    { 
	    ...
	    findwindow.style.position = 'sticky';
    ...
    findwindow.style.border = '0px' + find_window_border;
    ...
    findwindow.style.top = '200px';
	findwindow.style.left = '120px';
...
// This part creates the instructions and the "find" button
	string += '<div style="padding-top: 5px;">'
	+ '<form onsubmit="return false;"class="input-group"><input type="search" class="w-full border rounded bg-white shadow-lg py-2 px-2" size="500" maxlength="500" id="fwtext"'
	+ ' onchange="resettext();" placeholder="Введите текст для поиска" aria-describedby="basic-addon2"><span id="find_msg"><br/></span>'
  	+ '<div class="text-right my-2">'
	+ '<button type="button" class="bg-blue-400 hover:bg-blue-500 rounded-l px-2 py-1 text-white" value="" onclick="findprev();">Пред.</button>'
	+ '<button type="button" class="bg-blue-400 px-2 rounded-r py-1 text-white hover:bg-blue-500" value="" onclick="this.blur(); findit();">След.</button>'
  	+ '</div>';// ver 5.3 - 5/15/2015 - added this.blur();
	if (enable_site_search) { // Version 5.4
		string += '<br/><label class="hidden"><input type="radio" name="search_type" value="page" checked>Page</label>'+
		'<label class="hidden"><input type="radio" name="search_type" value="site" id="find_site_search">Site</label>'
       ;	
	}
	string += '</form></div>\n';
	// ^ ver 5.1 - 10/17/2014
